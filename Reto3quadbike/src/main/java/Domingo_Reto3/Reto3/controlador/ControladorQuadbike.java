...
import Domingo_Reto3.Reto3.servicio.ServiciosQuadbike;
...
public class ControladorQuadbike {
    ...
    @GetMapping("/all")
    public List<Quadbike> getBikes() {
        return servicio.getAll();
    }

    @GetMapping("/{id}")
    public Optional<Quadbike> getBike(@PathVariable("id") int bikeId) {
        return servicio.getBike(bikeId);
    }

    @PostMapping("/save")
    @ResponseStatus(HttpStatus.CREATED)
    public Quadbike save(@RequestBody Quadbike bike) {
